...
def test_miniprogram_auto_next_delay_has_three_user_options():
    ...
    assert "{ key: 'fast', label: '快', delay: 300 }" in ts
    assert "{ key: 'normal', label: '标准', delay: 500 }" in ts
    assert "{ key: 'slow', label: '慢', delay: 800 }" in ts
...
def test_web_auto_next_delay_has_matching_user_options():
    ...
    assert "{ key: 'fast', label: '快', delay: 300 }" in fab_js
    assert "{ key: 'normal', label: '标准', delay: 500 }" in fab_js
    assert "{ key: 'slow', label: '慢', delay: 800 }" in fab_js
