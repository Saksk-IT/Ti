...
@auth_api_bp.route('/web_login/mini_webview_url', methods=['POST'])
@jwt_required
@limiter.limit("60 per minute")
def api_web_login_mini_webview_url():
    """小程序：生成用于 web-view 的一次性登录跳转 URL（像素级 1:1 复刻 Web 前台）"""
    data = request.json or {}
    next_path = (data.get("next") or "").strip() or "/hub"

    # 只允许站内路径，避免开放重定向
    if "://" in next_path or next_path.startswith("//") or next_path.startswith("\\"):
        next_path = "/hub"
    ...
    try:
        ...
        sess = WebLoginService.create_session(meta=meta)
        WebLoginService.confirm_session(sid=str(sess["sid"]), nonce=str(sess["nonce"]), user_id=int(user_id))
        sess = WebLoginService.ensure_exchange_token(str(sess["sid"]))

        token = (sess.get("token") or "").strip()
        if not token:
            return jsonify({"status": "error", "message": "生成登录令牌失败"}), 500

        from urllib.parse import quote
        path = f"/web_login/exchange_redirect?token={quote(token)}&next={quote(next_path)}"
        return jsonify({
            "status": "success",
            "data": {
                "path": path,
                "token_expires_at": sess.get("token_expires_at"),
            }
        }), 200
    except Exception as e:
        current_app.logger.error(f"生成 web-view 登录 URL 失败: {e}", exc_info=True)
