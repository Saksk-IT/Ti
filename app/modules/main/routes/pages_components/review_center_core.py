...
def _count_available(
    *, uid: int, source_type: str, subject_id: int | None,
    ...
    if tf != "all":
        base += " AND q.type = :type_filter"
        params["type_filter"] = any_type_to_portable_type(tf)
    if isinstance(tag_ids, list):
        in_clause, in_params = _build_named_in("q.id", tag_ids, "tag")
        base += f" AND {in_clause}"
        params.update(in_params)

    row = db.session.execute(sa_text(base), params).fetchone()
    return int(row._mapping["cnt"] or 0) if row else 0


def _load_bank_meta(*, uid: int, bank_id: int) -> tuple[str, list, list] | None:
    ...
    row = db.session.execute(
        sa_text("SELECT id, name FROM user_question_banks WHERE id = :bid AND status = 1"),
        {"bid": int(bank_id)},
    ).fetchone()
    scope_name = str(row._mapping["name"] if row else f"题库{bank_id}")
    ...
    try:
        ...
        types = [
            portable_type_to_q_type((r._mapping["p_type"] or ""))
    ...
    *, data_tips: list, tab_urls: dict, base_url: str, ctx: dict,
    ...
    try:
        ...
        for t in data_tips or []:
            text_val = str(t if isinstance(t, str) else (t.get("text", "") if isinstance(t, dict) else "")).strip()
            if not text_val:
                continue
            ...
            if "：" in text_val and "（" in text_val:
                try:
                    qt = text_val.split("：", 1)[1].split("（", 1)[0].strip()
                except Exception:
                    qt = ""
            ...
            elif "背题模式" in text_val:
                action_label = "开始背题"
                action_url = memo_url
            elif "开始刷题" in text_val:
                action_label = "开始刷题"
                action_url = quiz_url
            elif "打乱题目" in text_val:
                action_label = "开启打乱题目"
                action_url = _url_with_params(
                    base_url,
                    {**ctx, "tab": "practice", "type": q_type, "tag": tag,
            ...
            tip_items.append({"text": text_val, "action_label": action_label, "action_url": action_url})
        return tip_items
    except Exception:
        return data_tips or []
...
def _review_center_page(kind: str):
    ...
    # ── 加载题库元数据 + 题型 + 标签 ──
    if source_type == "public":
        subjects_meta = _get_accessible_subject_rows(db.session, uid)
        ...
        try:
            ...
            tags = list_user_tags(db.session, int(uid), int(subject_id))
        except Exception:
            tags = []
    else:
        _types_tags = _load_bank_meta(uid=uid, bank_id=bank_id)
    ...
    tag_ids = None
    if source_type == "public":
        tag_ids = _load_public_tag_ids(db.session, int(uid), tag)
    else:
        tag_ids = _load_bank_tag_ids(db.session, int(uid), int(bank_id), tag)

    if kind == "tags" and isinstance(tag_ids, list) and len(tag_ids) == 0:
        warning = warning or "当前标签下暂无题目，请先在刷题页给题目打上该标签。"

    count_kw = dict(uid=uid, source_type=source_type, subject_id=subject_id,
    ...
    if tab == "search" and keyword:
        ...
        search_questions, search_total = load_search_results(
            conn=db.session, uid=uid, source_type=source_type, subject_id=subject_id,
