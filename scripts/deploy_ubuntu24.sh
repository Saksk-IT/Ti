...
if [[ "$DEPLOY_ENV" == "production" ]]; then
  ...
  DEFAULT_SSE_ENABLED="false"
  DEFAULT_SSE_RETRY_AFTER_SECONDS="300"
else
  DEFAULT_ENV_FILE="$ROOT_DIR/.env.development"
  DEFAULT_COMPOSE_FILE="$ROOT_DIR/compose.dev.yml"
  DEFAULT_TI_IMAGE="ghcr.io/saksk-it/ti:latest"
  DEFAULT_SSE_ENABLED="true"
  DEFAULT_SSE_RETRY_AFTER_SECONDS="30"
fi
...
REQUESTED_SSE_ENABLED="${SSE_ENABLED-}"
REQUESTED_SSE_RETRY_AFTER_SECONDS="${SSE_RETRY_AFTER_SECONDS-}"
TI_IMAGE="${TI_IMAGE:-$DEFAULT_TI_IMAGE}"
TI_IMAGE_PULL_POLICY="${TI_IMAGE_PULL_POLICY:-always}"
SSE_ENABLED="${SSE_ENABLED:-$DEFAULT_SSE_ENABLED}"
SSE_RETRY_AFTER_SECONDS="${SSE_RETRY_AFTER_SECONDS:-$DEFAULT_SSE_RETRY_AFTER_SECONDS}"
...
load_env_file() {
  ...
  if [[ -n "$REQUESTED_SSE_ENABLED" ]]; then
    SSE_ENABLED="$REQUESTED_SSE_ENABLED"
  else
    SSE_ENABLED="${SSE_ENABLED:-$DEFAULT_SSE_ENABLED}"
  fi
  if [[ -n "$REQUESTED_SSE_RETRY_AFTER_SECONDS" ]]; then
    SSE_RETRY_AFTER_SECONDS="$REQUESTED_SSE_RETRY_AFTER_SECONDS"
  else
    SSE_RETRY_AFTER_SECONDS="${SSE_RETRY_AFTER_SECONDS:-$DEFAULT_SSE_RETRY_AFTER_SECONDS}"
  fi
  case "${SSE_ENABLED,,}" in
    true|false|on|off|1|0|yes|no) ;;
    *) fail "SSE_ENABLED 只能是 true/false/on/off/1/0/yes/no，当前值：${SSE_ENABLED}" ;;
  esac
  [[ "$SSE_RETRY_AFTER_SECONDS" =~ ^[0-9]+$ && "$SSE_RETRY_AFTER_SECONDS" -ge 1 ]] || fail "SSE_RETRY_AFTER_SECONDS 必须是正整数"

  export TI_IMAGE TI_IMAGE_PULL_POLICY HTTP_BIND HTTP_PORT SESSION_COOKIE_SECURE SSE_ENABLED SSE_RETRY_AFTER_SECONDS
  POSTGRES_USER="${POSTGRES_USER:-studyuser}"
  POSTGRES_DB="${POSTGRES_DB:-ti_db}"
  DEFAULT_ADMIN_USERNAME="${DEFAULT_ADMIN_USERNAME:-admin}"
  DEFAULT_ADMIN_PHONE="${DEFAULT_ADMIN_PHONE:-}"
...
prepare_runtime_files() {
  ...
  upsert_env_value "SSE_ENABLED" "$SSE_ENABLED"
  upsert_env_value "SSE_RETRY_AFTER_SECONDS" "$SSE_RETRY_AFTER_SECONDS"
  if [[ "$DEPLOY_ENV" == "production" ]]; then
