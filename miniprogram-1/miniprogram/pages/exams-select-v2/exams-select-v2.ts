...
const KEY_TAB = 'exams_bank_tab';
const KEY_KW = 'exams_bank_kw';
...
Page({
  ...
  onPublicBankTap(e: any) {
    ...
    if (!name) return;
    const url = `/pages/index-v2/index-v2?tab=new&source=public&subject=${encodeURIComponent(name)}`;
  ...
  onBankTap(e: any) {
    ...
    const url = `/pages/index-v2/index-v2?tab=new&source=user_bank&bank_id=${encodeURIComponent(String(id))}`;
    safeNavigate(url, 'navigateTo');
  },
