...
function buildCampusOverview(payload, dateInput) {
    var scheduleRows = (0, campus_content_1.normalizeScheduleSnapshots)(Array.isArray(payload.snapshots) ? payload.snapshots : []);
    var gradeRows = (0, campus_content_1.normalizeGradeSnapshots)(Array.isArray(payload.grade_snapshots) ? payload.grade_snapshots : []);
    var today = (0, campus_content_1.buildTodayScheduleSummary)(scheduleRows, dateInput);
    var latestGrade = (0, campus_content_1.buildLatestGradeSummary)(gradeRows);
    return {
...
function buildCampusSummary(payload, isLoggedIn, dateInput) {
    if (dateInput === void 0) { dateInput = new Date(); }
    var data = payload && typeof payload === 'object' ? payload : {};
    var overview = isLoggedIn ? buildCampusOverview(data, dateInput) : emptyCampusOverview();
    if (!isLoggedIn) {
        return __assign(__assign({ title: '校园服务', subtitle: '登录后绑定教务系统账号，查询课表和成绩', statusLabel: '登录后使用', statusTone: 'muted' }, overview), { primaryAction: '去登录', secondaryAction: '进校园' });
    }
    if (data.error) {
        return __assign(__assign({ title: '校园服务', subtitle: '校园状态暂时无法同步，仍可进入校园页查看', statusLabel: '同步失败', statusTone: 'warn' }, overview), { primaryAction: '进入校园', secondaryAction: '稍后重试' });
    }
    ...
    if (hasCredentials) {
        return __assign(__assign({ title: '校园服务', subtitle: "\u5DF2\u7ED1\u5B9A ".concat(usernameHint, "\uFF0C\u53EF\u76F4\u63A5\u67E5\u8BE2\u8BFE\u8868\u548C\u6210\u7EE9"), statusLabel: '已绑定', statusTone: 'ok' }, overview), { primaryAction: '查课表', secondaryAction: '查成绩' });
    }
    return __assign(__assign({ title: '校园服务', subtitle: '绑定教务系统账号后，可在校园页查询课表和成绩', statusLabel: '待绑定', statusTone: 'warn' }, overview), { primaryAction: '去绑定', secondaryAction: '进校园' });
}
