...
const KEY_TAB = 'tags_bank_tab';
const KEY_KW = 'tags_bank_kw';
...
Page({
  ...
  onPublicBankTap(e: any) {
    ...
    const name = e?.currentTarget?.dataset?.name ? String(e.currentTarget.dataset.name) : '';
    let subjectName = name;
    if (!subjectName) {
      const hit = (this.data.subjects || []).find((s: any) => Number(s?.id || 0) === id);
      subjectName = hit ? String(hit.name || '') : '';
    }
    if (!subjectName) return;
    const url = `/pages/review-center-v2/review-center-v2?kind=tags&subject=${encodeURIComponent(subjectName)}`;
  ...
  onBankTap(e: any) {
    const id = Number(e?.currentTarget?.dataset?.id || 0);
    if (!Number.isFinite(id) || id <= 0) return;
    const url = `/pages/review-center-v2/review-center-v2?kind=tags&bank_id=${encodeURIComponent(String(id))}`;
    safeNavigate(url, 'navigateTo');
  },
