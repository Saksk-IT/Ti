...
const CHART_IDS: string[] = ['dcMistakeTrendChart', 'dcMistakeTopChart', 'dcMistakeDifficultyChart', 'dcMistakeTypeChart'];
...
Page({
  ...
  onLoad(options: any) {
    ...
    const url = `/packages/data/pages/data-center-v2/data-center-v2?tab=mistakes&days=${encodeURIComponent(String(days))}`;
    wx.redirectTo({
      url,
      fail: () => {
  ...
  onGoQuizPublicMistakes(e: any) {
    ...
    if (!subject) return;
    const url =
      `/pages/quiz/quiz?mode=quiz&source=mistakes&subject=${encodeURIComponent(subject)}` + (qType ? `&type=${encodeURIComponent(qType)}` : '');
    safeNavigate(url, 'navigateTo');
  },
  ...
  async loadStats(force = false) {
    ...
    try {
      ...
      const built = buildMistakesViewModel(res, this.data.days);
      self.__dcPayload = built.payload;
