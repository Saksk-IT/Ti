...
export function getCachedDataTags(days: any): any | null {
...
export function setCachedDataTags(days: any, res: any): void {
  const key = toKey(days);
  if (!key) return;
  cache[key] = { at: Date.now(), res };
}

export function clearCachedDataTags(days?: any): void {
  if (days == null) {
