...
exports.api = exports.normalizeImageUrls = exports.resolveUploadUrl = exports.getApiOrigin = void 0;
// API基础配置
    ...
    if (raw.startsWith('[')) {
        try {
            ...
            if (typeof parsed === 'string') {
                var url_1 = resolveUploadUrl(parsed);
                return url_1 ? [url_1] : [];
            }
        }
        catch (e) {
...
function safeLogApiResponse(method, url, statusCode, data) {
    try {
        ...
        try {
            console.log("API\u8BF7\u6C42 ".concat(method, " ").concat(url, ":"), statusCode, '[log skipped]');
        }
        catch (e2) { }
    }
}
function request(url, method, data) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        if (url.includes('/quiz/subjects')) {
            console.log('API请求token状态:', token ? "\u6709token(".concat(token.substring(0, 20), "...)") : '无token');
        }
...
exports.api = {
    ...
    // 微信：未绑定时创建新账号
    wechatCreate: function (wechatTempToken) {
        return request('/wechat/create', 'POST', { wechat_temp_token: wechatTempToken });
    },
    // 微信：绑定已有账号（邮箱验证码）
    ...
    getSubjectQuestions: function (subject, params) {
        return request("/quiz/subjects/".concat(encodeURIComponent(subject), "/questions"), 'GET', params || {});
    },
    // 科目收藏新增趋势（按收藏创建时间聚合）
    ...
    getHistoryStats: function (days) {
        if (days === void 0) { days = 30; }
        return request('/quiz/history', 'GET', { days: days });
    },
    ...
    getDataTags: function (days) {
        if (days === void 0) { days = 30; }
        return request('/data/tags', 'GET', { days: days });
    },
    ...
    getBankStatsDetail: function (bankId, daysOrParams) {
        if (daysOrParams === void 0) { daysOrParams = 14; }
        var params = typeof daysOrParams === 'number' ? { days: daysOrParams } : (daysOrParams || {});
        return request("/user/banks/api/".concat(bankId, "/stats"), 'GET', params);
    },
    // 题库收藏新增趋势（按收藏创建时间聚合）
