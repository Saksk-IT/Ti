...
var MemoryCache = /** @class */ (function () {
    ...
    MemoryCache.prototype.set = function (key, value, ttlMs) {
        if (ttlMs === void 0) { ttlMs = 0; }
        var ttl = Number(ttlMs) || 0;
        var expiresAt = ttl > 0 ? Date.now() + ttl : 0;
        this.store.set(key, { value: value, expiresAt: expiresAt });
    };
    MemoryCache.prototype.del = function (key) {
        this.store.delete(key);
    };
    MemoryCache.prototype.clear = function (prefix) {
        var _this = this;
        if (!prefix) {
            this.store.clear();
            return;
        ...
        var p = String(prefix);
        Array.from(this.store.keys()).forEach(function (k) {
            if (k.startsWith(p))
                _this.store.delete(k);
        });
    };
