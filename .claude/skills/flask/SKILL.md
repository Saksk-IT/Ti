...
name: Flask开发
description: |
  使用应用工厂模式、Blueprints和Flask-SQLAlchemy构建Python Web应用。可预防9种已知错误，包括stream_with_context清理问题、async/gevent冲突和CSRF缓存问题。

  适用场景：创建Flask项目、组织Blueprints，或排查循环导入、上下文错误、注册、流式传输或认证问题。
user-invocable: true
---
