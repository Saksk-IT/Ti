...
- **阶段 4C：user-counts HTTP 入口门禁已闭合，下一项是获准的双 alias HTTP 实现。** 完整 use case 与 HTTP owner 固定为 `learning`，唯一跨模块依赖方向保持 `learning -> personalbank::api`；当前 HTTP-neutral 生产面仍是 27 个公开应用方法、40 个 `learning`/`personalbank` 主源码和 288 文件生产运行面。入口门禁新增的 62-case HTTP/CORS 旧栈证据与 7 组限流证据只固定观察，不冒充 Java HTTP 实现；批准差异 `P4C-LEARNING-007` 至 `012` 冻结凭据选择、`last_active` 零写、有效 actor 限流、API-only CORS/OPTIONS、Unicode `Nd` 路径和 HEAD 零体。独立 Python/Java successor bridge 对除自身 provenance 哈希外的完整合同做固定规范化指纹，并显式承接 composition→read→HTTP 三层历史源，不允许 builder、证据或 future allowlist 自签扩权。全部 source tools 468/468、Phase 1 23/23、Phase 2、Phase 3 29/29、拓扑 60/60 与完整 `clean verify` 545 个 surefire + 79 个 failsafe 均通过，0 failure/error/skip。当前 `server/src/main`、生产资源、OpenAPI 与 route 状态相对 read predecessor 仍为零变更，保持 11 migrated、600 pending、0 cutover；operator、schema/index、真实迁移与生产切流继续禁止。下一精确生产切片完成后必须生成新的追加式 WORM，不能复用当前 read WORM。
...
- 第二层固定 successor bridge 同时绑定八个历史 Python 测试、四个 Java parity 测试、进度交接和 composition predecessor；历史控制面不以扩大的当前树重算旧事实。WORM 以四节点追加链固定最终 build-context，前三份报告未覆盖；读取合同保持 27 方法、40 模块文件、288 运行文件、11 migrated、600 pending、0 cutover。
- 已闭合 user-counts HTTP 入口证据与授权合同：固定旧提交的 62 个 HTTP case 覆盖双 alias、GET/HEAD/OPTIONS、认证、首参数、路径整数、协商与 8 个 CORS 观察；case/document payload SHA-256 分别为 `f577ff99a7f04030fd5f4dae0f95610351d4fcfff92de7e9ca0c406516725dbf` / `3e8f7c24548d979723d2601c11221b9e569de7b342e6c3c0d8daa25de74cdd2f`。独立限流证据确认 base `10/second;500/hour;5000/day`、固定生产默认 100 倍、alias 独立桶、旧 Session/Bearer/IP key 缺陷与 Redis 拒绝 500；目标差异改为权威 actor HMAC 和 fail-closed 503，但当前仍未实现。
...
- HTTP 入口门禁最终验证为：专用 evidence/read/composition/entry Python 链全绿，四个历史 Java parity 24/24；全部 source tools 468/468；Phase 1 23/23、Phase 2 static、Phase 3 29/29、topology 60/60；单线程 `clean verify` 545+79，0 failure/error/skip。门禁只改测试、工具、证据与文档，没有生成新 WORM，也没有修改任何生产 Java/资源、OpenAPI 或 route 状态。
...
1. 实现入口合同唯一授权的两条 user-counts alias：新增精确的 learning-owned Controller/security error writer、请求解析器、独立三窗口 Redis limiter/filter、API-only CORS source；只修改 `SecurityConfiguration`、`LoginRateLimitConfiguration`、`application.yml`、`application-prod.yml`、`.env.example` 与 `compose.dev.yml`，新增单一 Phase 4C OpenAPI overlay 和两行 route delta。必须完成真实 Servlet 的认证/CORS/HEAD/路径矩阵、Redis 并发/TTL/故障、PG16.14/18.4 HTTP/SQL/指纹、59-case approved-difference parity、生产面清单与新追加式 WORM；实现结束后只登记 13 migrated、598 pending、0 cutover，禁止 identity API/global auth filter、learning/personalbank persistence、schema/index、operator、真实迁移、客户端/网关和生产切流改动。
...
5. 后续每个切片都必须重新执行并记录与其风险相称的全量静态、契约、Maven、WORM 和独立抽取门禁；不得复用本轮 545+79 或 build-context SHA-256 `935e6a95a33621b01e1e04d752a09513c8037cffe807a73fa1ce9850fb5912f0` 冒充新切片证据。
