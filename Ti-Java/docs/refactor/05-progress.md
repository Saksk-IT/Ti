...
- **阶段 4C：user-counts 组合契约、有效数据所有权覆盖和逐行事务迁移原语证据已闭合，只授权下一提交实现 HTTP-neutral Java 内部能力。** 完整 use case 与未来 HTTP owner 固定为 `learning`，唯一允许的跨模块依赖方向为既有的 `learning -> personalbank::api`；历史 `bank_<bank_id>_tags` 命名空间的有效 owner 从 `personalbank` 覆盖为 `learning`，159 个资源仍全部唯一归属。契约冻结未来 27 个公开应用方法的精确形状（当前 23 个；新增 `learning` 1 个、`personalbank` 3 个），并以逐行锁、目标优先、insert-only、行级回滚、二次扫描零 DML 和 schema/index/source 指纹不变闭合测试原语。该证据不等于全局 dry-run/preflight 或生产 operator：真实数据执行、全局 all-or-block、并发锁竞争/冲突竞态、生产迁移、Controller、Security matcher、route/OpenAPI delta、schema/index 与生产切流均保持未授权。Phase 4C 契约链 51/51、全部 source tools 423/423、迁移原语 unit 11/11、PG16.14/18.4 集成 2/2，以及 UTC 环境下完整 `clean verify` 514 个 surefire + 76 个 failsafe 均通过，0 failure/error/skip；当前生产 shape 仍为 23 个公开方法、11 migrated、600 pending、0 cutover，且未新增任何生产 Java。
...
| Phase 4C user-counts 组合、owner overlay 与逐行事务原语 | Phase 4C 契约链 51/51；全部 source tools 423/423；迁移原语 unit 11/11；PG16.14/18.4 集成 2/2；UTC `clean verify` 514+76；1 项有效 owner 覆盖且资源总数保持 159 | 组合契约与测试原语绿色，只授权下一提交的精确 27-method HTTP-neutral Java shape；全局 preflight/operator、真实数据、生产 Java、HTTP/security/route/OpenAPI、schema/index 与 cutover 增量均为 0 |
