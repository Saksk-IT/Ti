...
- **阶段 4C：user-counts HTTP 实现已存在，但完整目标 parity 尚未闭合，两条 GET 继续 pending。** 双 alias Controller、安全错误 writer、Unicode `Nd` 路径解析、API-only CORS/OPTIONS、独立 Redis 三窗口限流、配置、OpenAPI 和 route delta 已物化。完整 use case 与 HTTP owner 固定为 `learning`，跨模块方向保持 `learning -> personalbank::api`。当前 59-case 证据明确是 48 个 MockMvc + 8 个 authentication binding + 3 个 typed PostgreSQL binding 组成的 `PARTIAL_EXECUTION_MAPPING_LEDGER`；`full_target_parity_closed=false`、`route_migration_eligible=false`。新限流 namespace 已纳入 `learning` 所有权 successor，有效资源为 160/160。当前有效路由状态保持 **11 migrated、600 pending、0 cutover**；operator、schema/index、真实迁移与生产切流继续禁止。
- 最近已推送的绿色提交为 `273ee92`（`test(java): gate user counts http entry`）；当前 user-counts HTTP pending implementation 仍是未提交工作树，不得把它记作已提交检查点。
- 当前 implementation successor 覆盖 297 个 production runtime files，相对 288 文件历史 predecessor 精确新增 9 个、修改 6 个，并绑定 44 个 source contract 路径；第五节点 WORM 报告为 `7b863dd3…ca39`，Java build-context 为 `27322797…8a3`。这些结构化事实不改变 59-case PARTIAL 与 11/600/0 状态。
...
- HTTP entry successor trust root 精确固定证据、批准差异、目标 HTTP 语义、8 个未来新主源码、2 个允许修改的主源码、4 个配置文件、单一 OpenAPI overlay、禁止文件和零切流边界；只把 Python/Java bridge 自身物理 SHA 作为已校验 provenance 从规范化指纹中替换，避免自/交叉哈希环。CORS 依 Spring Security 顺序在认证前处理，路径边界明确 literal slash 形成 extra segment 404、encoded slash 与 literal/encoded semicolon 由 firewall 400。
...
- 有效路由状态现为 **11 migrated、600 pending、0 production cutover**；两条 user-counts GET 是 600 pending 中的 implemented-pending 子集，派生 HEAD/OPTIONS 不计 operation。有效资源为 **160 个且 160 个均有唯一 owner**。旧 Flask 仍是生产 owner，整个长期重构目标仍未完成。
...
上表保留 Phase 4A 各历史切片并记录最终 closure，同时区分 Phase 4B personalbank 分类最终证据，以及分享列表、全部分享和 usage-stats 各自的实现前入口与内部实现。分类的 424+60/WORM/独立清单、分享列表入口的 429+62、全部分享入口的 451+66、全部分享实现的 317 项 source tools/465+68 Maven，以及 usage-stats 入口的 359 项 source tools/470+70 Maven 均不得冒充 usage-stats 实现验收；usage-stats 实现以定向 unit 30、PG runtime adapter 2、全部 source tools 364 与 Maven 494+72 独立闭合，但仍不授权 HTTP、schema/index 或生产切流。Phase 4B user-counts 的 413 项 source tools 与 503+74 Maven 只闭合实现前证据和 owner 决策；Phase 4C 的 59 项组合/迁移合同与 517+76 Maven 只授权 HTTP-neutral shape。内部读取的第二层合同 77、全部 source tools 442、双 PG IT、四节点 WORM、Maven 545+79 与 `935e6a95…12f0` 均是历史 HTTP-neutral read predecessor 证据；HTTP entry 的 468 项 source tools 和再次 545+79 Maven 只授权当前精确适配实现。297 文件/44 source contract successor 的 pre-documentation checkpoint 通过了 1,449 文件独立验收、496 项 source tools 与 661+93 Maven；该冻结 manifest 不代表当前工作树字节，后续证据/trust/parity 更新由当前合同门禁约束。这些证据闭合可抽取构建/测试/启动边界，但现有 pending implementation 检查点仍不证明 59-case 全目标等价、完整认证链、路由迁移、生产 operator、全局 preflight、真实数据迁移或生产切流完成。

完整命令、两个 pytest 失败说明及初步性能数字见 `07-baseline-results.md`。

## 尚未迁移的路由与数据

- **路由：** Phase 0 冻结矩阵仍保存 592 条规则/611 个 operation 的历史 `pending` 事实；Phase 3 与 Phase 4A delta 已把认证 2 条、受保护科目目录 2 条和公共题库 GET 7 条物化为 **11 个 `migrated`**，其余 **600 个 operation 仍 pending**。两条 user-counts GET 是其中的 implemented-pending 子集，派生 HEAD/OPTIONS 不计 operation。生产切换数为 0，旧 Flask 仍是当前运行所有者。
...
- user-counts 的 59-case 映射目前只是 `PARTIAL_EXECUTION_MAPPING_LEDGER`：48 个 MockMvc case mock 了应用 API，8 个 case 仅绑定认证测试，3 个 case 仅绑定数据库测试。这些证据不能替代 Target Session、Flask Session、Bearer 经真实过滤链到 Controller/应用/JDBC 的端到端执行，也尚未覆盖所有前置终止的 `users.last_active` 零 DML 与原始 Servlet 路径指纹。在此风险闭合前，两条 GET 必须继续 pending。

## 下一项具体动作

1. 闭合当前 user-counts implemented-pending 检查点：把 59 个 golden case 全部转换为真实目标执行，消除 `BOUND_*` 和 mocked-result 回显式证明；完成 Target Session、Flask Session、Bearer 到 Controller/应用/JDBC 的完整链路，补齐全部前置终止的 `users.last_active` 零 DML、原始 Servlet Unicode `Nd`/溢出/near-miss 指纹，并重新验证真实 Tomcat、Redis 拒绝/中断/同客户端恢复、PG16.14/18.4、固定 successor、第五节点 WORM 与独立抽取。在此门禁闭合前始终保持 **11 migrated、600 pending、0 cutover**；禁止 identity API/global auth filter、learning/personalbank persistence、schema/index、operator、真实迁移、客户端/网关和生产切流改动。
