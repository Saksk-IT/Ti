...
- **阶段 4C：user-counts HTTP-neutral Java 读取已实现并闭合，下一项是两条 alias 的 HTTP 入口合同。** 完整 use case 与未来 HTTP owner 固定为 `learning`，唯一跨模块依赖方向保持 `learning -> personalbank::api`；新增 17 个 `learning`/`personalbank` 主源码并只修改 `LearningApplicationApi`，累计形成 27 个公开应用方法、40 文件模块清单与 288 文件生产运行面。组合实现以 `NOT_SUPPORTED` 暂停外层事务，learning memberships 与 personalbank facts 各用独立 `REQUIRES_NEW` 只读事务；任一 `DENIED` 终止并丢弃部分结果，favorites/mistakes/types 只有基础设施或事务查询异常才字段级降级。分享访问严格绑定 requested/share-record/share 三方同题库，只接受 `read`/`copy`，空或未知权限 fail closed；PG16.14/18.4 IT 还证明 optional 阶段自身进入 25P02 后后续字段从干净事务继续。第二层历史合同 77/77、全部 source tools 442/442、完整 `clean verify` 545 个 surefire + 79 个 failsafe 均通过，0 failure/error/skip；第四个追加式 WORM tip SHA-256 为 `a393e79afb76c53a1aca8be1e4709506b58ad062e3c6536c26c12f10b29d1ec6`，绑定 Java build-context `935e6a95a33621b01e1e04d752a09513c8037cffe807a73fa1ce9850fb5912f0`，前三份报告保持不可变。当前状态仍是 11 migrated、600 pending、0 cutover；Controller、Security matcher、route/OpenAPI、schema/index、真实迁移和生产切流均未新增。历史 tag owner overlay 与逐行迁移原语仍只是证据；生产 operator 的 session advisory lock、全局 all-or-block preflight、写冻结/版本复核和持久 ledger/tombstone 尚未闭合。
...
1. 建立两条 user-counts alias 的 Phase 4C HTTP 入口合同：从固定调用方与 59-case golden 精确冻结条件认证、`q_type/source/tag` 首值与默认值、legacy 信封/状态码、缓存/限流、GET 零 DDL/DML 和故障投影，并把当前 27-method HTTP-neutral read 作为不可变 predecessor。入口门禁本身不得新增 Controller、Security matcher、route/OpenAPI delta、schema/index、真实数据迁移或生产切流；只有该合同绿色后才能另行授权 HTTP 适配实现。
