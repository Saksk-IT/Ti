...
- **阶段 4C：user-counts 组合契约、有效数据所有权覆盖和逐行事务迁移原语证据已闭合，只授权下一提交实现 HTTP-neutral Java 内部能力。** 完整 use case 与未来 HTTP owner 固定为 `learning`，唯一允许的跨模块依赖方向为既有的 `learning -> personalbank::api`；历史 `bank_<bank_id>_tags` 命名空间的有效 owner 从 `personalbank` 覆盖为 `learning`，159 个资源仍全部唯一归属。契约冻结未来 27 个公开应用方法的精确形状（当前 23 个；新增 `learning` 1 个、`personalbank` 3 个），并以逐行锁、严格源/目标校验、insert-only、行级回滚、提交结果分类、二次扫描零 DML 和 schema/index/source 指纹不变闭合测试原语。完整迁移设计仍未闭合：生产 operator 还必须补齐 session-level advisory lock、全局 all-or-block preflight、源/目标/membership 写冻结或版本复核，以及区分首次迁移与目标删除的持久 ledger/tombstone；真实数据执行、Controller、Security matcher、route/OpenAPI delta、schema/index 与生产切流均保持未授权。Phase 4C 契约链 59/59、全部 source tools 431/431、迁移原语 unit 14/14、PG16.14/18.4 集成 2/2，以及完整 `clean verify` 517 个 surefire + 76 个 failsafe 均通过，0 failure/error/skip；入口 WORM 报告 SHA-256 为 `cfb262319ded0840218fd9bfb4deff1e7bc9c66b5849e3ff05f49a459e686884`，绑定 271 文件生产面 manifest `7d6113701aac8268f22e8b58b3c52d7d8ea388ddaa06aa2d3d7bd334edd17ebd` 与 Java build-context `c59ee688646b7c23f0f883b4c1377d2a33b507e7dd08b978e98cf3ebdc11825c`。当前生产 shape 仍为 23 个公开方法、11 migrated、600 pending、0 cutover，且未新增任何生产 Java。
...
1. 按 Phase 4C composition contract 实现精确的 27-method HTTP-neutral Java shape：由 `learning` 新增完整 user-counts 请求/结果与组合服务，由 `personalbank::api` 新增三个最小题库/题目事实能力；保持 `learning -> personalbank::api` 单向依赖、当前 23-method manifest 前置校验、事务与 optional-query 失败边界。该提交不得包含 Controller、Security matcher、route/OpenAPI delta、schema/index、真实数据迁移或生产切流。
