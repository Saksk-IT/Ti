...
1. 限定暂存 `Ti-Java/` 并审计清单，提交 `docs(refactor): inventory legacy contracts` 后推送 `main`；不得带入根 `AGENTS.md`、`CLAUDE.md`、`.playwright-cli/`、`miniprogram-1/` 或 `output/`。
2. 回填阶段 0 绿色提交，将“当前阶段”切换到阶段 1，再次限定提交和推送进度文档。
3. 阶段 1 先完成 7 类 ADR、统一 API 契约、模块公开 API/表所有权与无环 DAG，再从 592 条真实路由生成 OpenAPI 3.1 初稿与契约门禁。
