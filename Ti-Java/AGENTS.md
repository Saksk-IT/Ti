...
本节是 Wang 对根目录旧“所有任务都直接在 `main` 工作”规则的显式调整；在 `Ti-Java/` 并行重构期间，以本节、历史基线 `docs/refactor/parallel/coordination-contract.md` 和其追加式 successor `docs/refactor/parallel/coordination-contract-v2.md` 为准。新波次必须使用 v2；v1 只保留为历史恢复点：
...
- 每个 v2 Worker 只能使用仓库外只读 wave assignment 中登记的一个或多个精确 `ownership_targets`；目标之间及跨 lane 必须按路径段互不重叠。其唯一额外可写路径是 assignment 固定的 `docs/refactor/parallel/handoffs/<lane>.json`，且只能出现在实现提交之后的 handoff-only 提交中。
...
完整中央文件清单、不可变 assignment、机器验证器、lane/handoff 合同、锁路径、锁顺序和集成拒绝条件见 `docs/refactor/parallel/coordination-contract-v2.md`。v2 Worker 的聊天说明、分支 tip 或手工清单都不能替代验证器通过的固定实现 SHA 与 handoff SHA。
