...
Page({
  ...
  async loadTags() {
    try {
      const res: any =
        this.data.sourceType === 'bank' && this.data.bankId
          ? await api.getBankTags(this.data.bankId)
          : await api.getTags({ subject: this.data.subject });
  ...
  async loadAvailableTypesAndStats() {
    ...
    try {
      ...
      // 按常见题型顺序排序，其它题型置后
      const preferredOrder = ['选择题', '多选题', '判断题', '填空题', '简答题', '计算题'];
  ...
  buildProgressKey(mode: 'quiz' | 'memo'): string {
    ...
    const shuffleO = (this.data.settings.shuffleOptions && this.isOptionShuffleAllowed(type)) ? '1' : '0';
  ...
  onSettingChange(e: any) {
    ...
    if (setting === 'shuffleOptions' && !this.isOptionShuffleAllowed(this.data.selectedType)) {
      this.setData({ settings: { ...this.data.settings, shuffleOptions: false } });
      return;
    }
  ...
  isOptionShuffleAllowed(type: string) {
    if (!type || type === 'all') return true;
    return type === '选择题' || type === '多选题';
  },

  syncShuffleOptionAvailability(type?: string) {
    const targetType = (type !== undefined ? type : this.data.selectedType) || 'all';
    const canShuffleOptions = this.isOptionShuffleAllowed(targetType);
  ...
  saveSettings(settings: { shuffleQuestions: boolean; shuffleOptions: boolean }) {
    try {
