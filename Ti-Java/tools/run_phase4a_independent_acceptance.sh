...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/ti-phase4a-independent.XXXXXX")"
...
PROJECT="ti-p4a-$LOWER_STAMP"
IMAGE="ti-java-phase4a:$LOWER_STAMP"
MAVEN_CACHE="ti-java-phase4a-m2-$LOWER_STAMP"
...
LOCK_DIR="$REPORT_ROOT/phase4a-independent-acceptance.lock"
...
if ! mkdir -- "$LOCK_DIR"; then
    printf '%s\n' 'Another Phase 4A independent acceptance run holds the lock' >&2
    exit 1
fi
LOCK_HELD=true
...
verify_docker_client_isolation
if [ -z "$REPORT" ]; then
    REPORT="$REPORT_ROOT/phase4a-independent-acceptance-$STAMP.json"
fi
...
make_manifest() {
    manifest_root="$1"
    manifest_output="$2"
    python3 - "$manifest_root" "$LIST0" "$manifest_output" <<'PY'
import hashlib
import json
import os
import pathlib
...
(
    cd "$SOURCE"
    TI_PHASE4A_PREFINAL_ACCEPTANCE=1 \
        TI_PHASE4A_PREFINAL_LOCK_TOKEN="$LOCK_TOKEN" \
...
if not match or int(match.group(1)) != 215 or not re.search(r"^OK$", text, re.MULTILINE):
    raise SystemExit("canonical frozen-source tests did not report 215/215")
PY
...
output = pathlib.Path(sys.argv[2])
expected = {"surefire-reports": 406, "failsafe-reports": 58}
summary = {}
...
[ -d "$REPORT_ROOT" ] && [ ! -L "$REPORT_ROOT" ]
REPORT_TEMP="$(mktemp "$REPORT_ROOT/.phase4a-independent-report.XXXXXX")"
...
jq -n \
    ...
    '{
        schemaVersion: 1,
        status: "passed",
        scope: "phase4a-prefinal-independent-copy",
        ...
        independentCopy: {
            ...
            maven: {
                surefire: {tests: 406, failures: 0, errors: 0, skipped: 0},
                failsafe: {tests: 58, failures: 0, errors: 0, skipped: 0}
            },
...
printf 'Phase 4A prefinal independent acceptance passed\n'
printf 'report=%s\n' "$REPORT"
