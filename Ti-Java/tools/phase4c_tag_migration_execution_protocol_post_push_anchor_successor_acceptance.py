#!/usr/bin/env python3
"""Independent Gitless acceptance for the Phase 4C execution-protocol D2 anchor."""
...
EXPECTED_CONTRACT_BYTE_COUNT = 80_324

_TOP_LEVEL_KEYS = {
    "contract_id", "schema_version", "captured_at", "status", "scope",
    "execution_protocol_contract", "independent_acceptance_evidence",
    "implementation_checkpoint", "independent_acceptance_checkpoint",
    "execution_protocol_authority_anchor", "transitive_node_c_anchor",
...
def _regular_anchored_file(root: Path, relative: str) -> Path:
    allowed = set(builder.D0_CHANGES) | set(builder.D1_CHANGES) | {
        CONTRACT_RELATIVE,
    }
    value = Path(relative)
    if value.is_absolute() or relative not in allowed:
        raise AssertionError("execution-protocol D2 unknown or absolute anchor path")
    ...
    for part in value.parts:
        ...
        if cursor.is_symlink():
            raise AssertionError(
                f"execution-protocol D2 anchor path is a symlink: {relative}"
            )
    try:
        resolved = candidate.resolve(strict=True)
    except (OSError, RuntimeError) as error:
        raise AssertionError("execution-protocol D2 anchored source missing") from error
    if not resolved.is_relative_to(base) or not resolved.is_file():
        raise AssertionError("execution-protocol D2 anchored source escaped root")
    return resolved
...
def validate_contract(document: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise AssertionError("execution-protocol D2 contract identity/shape drifted")

    if document.get("execution_protocol_contract") != {
        "source": builder.EXECUTION_PROTOCOL_CONTRACT_RELATIVE,
        "contract_id": builder.EXECUTION_PROTOCOL_CONTRACT_ID,
        "sha256": builder.EXECUTION_PROTOCOL_CONTRACT_SHA256,
        "byte_count": builder.EXECUTION_PROTOCOL_CONTRACT_BYTE_COUNT,
        "document_payload_sha256": builder.EXECUTION_PROTOCOL_CONTRACT_PAYLOAD_SHA256,
        "immutable": True,
    }:
        raise AssertionError("execution-protocol D2 predecessor descriptor drifted")
    ...
    if (
        ...
        raise AssertionError("execution-protocol D2 independent evidence drifted")

    d0 = document.get("implementation_checkpoint", {})
    d1 = document.get("independent_acceptance_checkpoint", {})
    d0_artifacts = d0.get("artifacts", {})
    d1_artifacts = d1.get("artifacts", {})
    if (
        {key: value for key, value in d0.items() if key != "artifacts"}
        != builder.D0_CHECKPOINT
        or d0_artifacts != builder.D0_CHANGES
        or {key: value for key, value in d1.items() if key != "artifacts"}
        != builder.D1_CHECKPOINT
        or d1_artifacts != builder.D1_CHANGES
        or d1.get("parent_oid") != d0.get("commit_oid")
        or len(d0_artifacts) != 55
        or sum(item["change_type"] == "A" for item in d0_artifacts.values())
        != 18
        or sum(item["change_type"] == "M" for item in d0_artifacts.values())
        != 37
        or tuple(d1_artifacts) != builder.D1_CONTROL_SOURCES
        or d1_artifacts[builder.EVIDENCE_RELATIVE]["mode"] != "100644"
        or d1_artifacts[builder.RUNNER_RELATIVE]["mode"] != "100755"
    ):
        raise AssertionError("execution-protocol D2 fixed checkpoints drifted")
    if (
        d1_artifacts[builder.EVIDENCE_RELATIVE]["sha256"]
        != evidence.get("sha256")
        or d1_artifacts[builder.EVIDENCE_RELATIVE]["byte_count"]
        != evidence.get("byte_count")
        or d1_artifacts[builder.RUNNER_RELATIVE]["sha256"]
        != runner.get("sha256")
        or d1_artifacts[builder.RUNNER_RELATIVE]["byte_count"]
        != runner.get("byte_count")
        or d1_artifacts[builder.RUNNER_RELATIVE]["mode"]
        != runner.get("git_mode")
    ):
        raise AssertionError("execution-protocol D2 evidence/runner cross-anchor drifted")

    anchor = document.get("execution_protocol_authority_anchor", {})
    ...
    if (
        ...
        != builder.D0_CONTROL_SOURCES
        or anchor.get("implementation_control_source_count") != 7
        or anchor.get("implementation_fixed_non_control_source_count") != 48
        or anchor.get("implementation_transition_source_count") != 37
        or tuple(anchor.get("independent_acceptance_control_sources", ()))
        != builder.D1_CONTROL_SOURCES
        or anchor.get("independent_acceptance_control_source_count") != 2
        or controls & fixed
        or controls | fixed != set(builder.D0_CHANGES)
        or transitions
        ...
        raise AssertionError("execution-protocol D2 authority partition drifted")

    transitive = document.get("transitive_node_c_anchor", {})
    node_c_predecessor = transitive.get("predecessor", {})
    if (
        transitive.get("immutable") is not True
        or node_c_predecessor.get("source")
        != (
            "docs/refactor/phase4c/personal-bank-tag-migration-operator-core-"
            "post-push-anchor-contract.json"
        )
        or node_c_predecessor.get("fixed_commit_oid")
        != "4c47d1ea220ae9e310338bbf23b74d87d477e20f"
        or node_c_predecessor.get("sha256")
        != "0c7041de3dff57ccaadcb995447b4ae10342ce39dd31e03291eecc916a95d936"
        or node_c_predecessor.get("byte_count") != 84_461
        or node_c_predecessor.get("document_payload_sha256")
        != "fb82185d0b87b19df4ef3fb6b9e95636731f33b5da6d21e6e2287471996a4e64"
        or node_c_predecessor.get("immutable") is not True
    ):
        raise AssertionError("execution-protocol D2 transitive Node C anchor drifted")
    ...
    if (
        ...
        or original_closure.get("fixed_d0_independent_copy_acceptance_closed")
        is not True
        or original_closure.get("proves_only_commit") != builder.D0_COMMIT
        or original_closure.get("proves_d1_evidence_commit") is not False
        or original_closure.get("proves_d2_anchor_commit") is not False
        or original_closure.get("self_hash_embedded") is not False
    ):
        raise AssertionError("execution-protocol D2 independent-copy boundary drifted")
    ...
    if (
        ...
        raise AssertionError("execution-protocol D2 authorization/route drifted")
    ...
    if (
        ...
        or production.get("worm", {}).get("current_chain_node_count") != 9
        or production.get("accepted_runtime_file_count") != 307
        or production.get("current_runtime_file_count") != 311
        ...
        raise AssertionError("execution-protocol D2 production/WORM boundary drifted")
    ...
    if (
        ...
        or current.get("d2_commit_or_tree_identity_embedded") is not False
        or set(builder.CURRENT_CONTROL_SOURCES) & anchored
    ):
        raise AssertionError("execution-protocol D2 self-authority boundary drifted")

    acceptance = document.get("acceptance", {})
    if (
        acceptance.get("implementation_checkpoint_changed_path_count") != 55
        or acceptance.get("implementation_checkpoint_added_count") != 18
        or acceptance.get("implementation_checkpoint_modified_count") != 37
        or acceptance.get(
        ...
        != 48
        or acceptance.get("implementation_transition_count") != 37
        ...
        or acceptance.get("d2_self_anchor_complete") is not False
        ...
        raise AssertionError("execution-protocol D2 acceptance boundary drifted")
...
def load_contract(root: Path = ROOT) -> dict[str, Any]:
    ...
    if (
        ...
        raise AssertionError("execution-protocol D2 contract fixed bytes drifted")
    document = json.loads(payload)
    if not isinstance(document, dict):
        raise AssertionError("execution-protocol D2 contract must be a JSON object")
    ...
    if document != built or payload != builder.serialized_contract(built):
        raise AssertionError("execution-protocol D2 contract differs from Gitless builder")
    return document


def minimal_fixture_paths() -> tuple[str, ...]:
    return (
        builder.EXECUTION_PROTOCOL_CONTRACT_RELATIVE,
...
def checkpoint_paths() -> tuple[str, ...]:
    return tuple(builder.D0_CHANGES) + tuple(builder.D1_CHANGES)
...
def accepted_sha256(root: Path, relative: str) -> str | None:
    descriptor = builder.D0_CHANGES.get(relative) or builder.D1_CHANGES.get(
        relative
    )
...
def _run_fixed_git(repository_root: Path, *arguments: str) -> bytes:
    ...
    if any(argument in forbidden for argument in arguments):
        raise AssertionError("execution-protocol D2 acceptance forbids live Git refs")
    ...
    try:
        ...
        raise AssertionError(
            "execution-protocol D2 independent fixed Git replay failed"
        ) from error
...
def _assert_diff(
    ...
    if (
        ...
        raise AssertionError(f"execution-protocol D2 {label} Git diff drifted")
...
def _replay_checkpoint(
    ...
    if (
        ...
        raise AssertionError("execution-protocol D2 fixed commit object drifted")
    ...
    if facts != [
        ...
        raise AssertionError("execution-protocol D2 commit facts drifted")
    if _fixed_git_text(repository, "show", "-s", "--format=%T", parent) \
            != metadata["parent_root_tree_oid"]:
        raise AssertionError("execution-protocol D2 parent root tree drifted")
    for key, relative in {
        ...
        if (
            ...
            raise AssertionError(f"execution-protocol D2 tree drifted: {relative}")
    ...
    for path, item in changes.items():
        ...
        if (
            ...
            raise AssertionError(f"execution-protocol D2 current tree blob drifted: {path}")
        ...
        if (
            ...
            raise AssertionError(f"execution-protocol D2 current blob drifted: {path}")
        ...
        if item["change_type"] == "A":
            if _run_fixed_git(
                ...
                raise AssertionError(f"execution-protocol D2 added path existed: {path}")
            added_total += len(current)
        elif item["change_type"] == "M":
            if _fixed_git_text(
                repository, "rev-parse", f"{parent}:{repository_path}"
            ) != item["previous_git_blob_oid"]:
                raise AssertionError(f"execution-protocol D2 parent blob drifted: {path}")
            ...
            if (
                ...
                raise AssertionError(f"execution-protocol D2 parent bytes drifted: {path}")
            previous_total += len(previous)
            modified_current += len(current)
            modified_parent += len(previous)
        else:
            raise AssertionError("execution-protocol D2 unsupported change type")
    if (
        ...
        raise AssertionError("execution-protocol D2 checkpoint aggregates drifted")


def validate_fixed_git_checkpoints(
    repository_root: Path = ROOT.parent,
) -> None:
    """Replay fixed D0 and D1 without invoking the builder's replay."""
    repository = repository_root.resolve(strict=True)
    if Path(_fixed_git_text(
        repository, "rev-parse", "--show-toplevel"
    )).resolve() != repository:
        raise AssertionError("execution-protocol D2 repository root drifted")
    if _fixed_git_text(
        repository, "rev-parse", "--show-object-format"
    ) != "sha1":
        raise AssertionError("execution-protocol D2 Git object format drifted")
    ...
    if (
        ...
        raise AssertionError("execution-protocol D2 D0/D1 chain drifted")

    operator = json.loads(
        (repository / "Ti-Java" / builder.EXECUTION_PROTOCOL_CONTRACT_RELATIVE).read_bytes()
    )
    overrides = operator["historical_source_successors"]["overrides"]
    d0_artifacts = document["implementation_checkpoint"]["artifacts"]
    for path, override in overrides.items():
        item = d0_artifacts[path]
        if (
            item["change_type"] != "M"
            ...
            raise AssertionError(
                f"execution-protocol D2 transition replay drifted: {path}"
            )
...
def main() -> int:
    ...
    print(
        "phase4c execution-protocol D2 post-push anchor acceptance passed: "
