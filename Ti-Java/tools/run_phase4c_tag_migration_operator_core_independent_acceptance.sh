...
readonly FIXED_COMMIT="a70c365959e123950d30bff05adb4fabbb72d640"
readonly FIXED_PARENT="bbeb08efcccb0b9974dfefa2044aab43e0675f6f"
readonly FIXED_ROOT_TREE="e23d0e8551dc8fd3338ba0682154eebe4a9e9f2f"
readonly FIXED_TI_JAVA_TREE="38f6aafbeb7888a5539d6ab3b4b6b8d9ae522194"
readonly FIXED_SERVER_TREE="c4243039f367f7f917a3f9c85c190ebc5233a0fb"
readonly FIXED_SRC_MAIN_TREE="bdd88effe149c61fada2300a4ec85bb2a3fdaf1c"
...
readonly PARENT_ROOT_TREE="2df48a21e622d0e5e3731fe2617ddaedbf466866"
readonly PARENT_TI_JAVA_TREE="ce2c2035763ac4512fa2bcaaa73cacb255212756"
readonly PARENT_SERVER_TREE="931e1268a43465023e23b31d903d5d7b3219981d"
readonly PARENT_SRC_MAIN_TREE="21fe4902d57a11998502e63041b5a56fb039a090"
readonly PARENT_WEB_TREE="$FIXED_WEB_TREE"
readonly PARENT_MINIPROGRAM_TREE="$FIXED_MINIPROGRAM_TREE"

readonly EXPECTED_STANDARD_RAW_SHA256="6892e663c1d1b5572c28c8aa489fc5721af4b4dbe7771edbbef12cf870bd5371"
readonly EXPECTED_STANDARD_NUMSTAT_SHA256="a42f9a7d5da81d41b08204bd26ffdbfc4da728f2541d417967c107d2f14dd87c"
readonly EXPECTED_STANDARD_NAME_STATUS_SHA256="2de8de0fd74bc734fe8cd4acb93dd50471b8179eb87e148c7f66e57188565d1d"
readonly EXPECTED_NUL_RAW_SHA256="ed145d808516678306ea1610fac1428244df90162371727ba9c1e4936381a196"
readonly EXPECTED_NUL_NUMSTAT_SHA256="f7b183cb1df7833418c014cd0a2d2a622dd0be43165e1dfa3dc6da5a7891d023"
readonly EXPECTED_NUL_NAME_STATUS_SHA256="22eb3e088ab24c2fcdb8defad142a01f6ba5a36bb81d9f0b5aec380a178c8725"
readonly EXPECTED_RAW_BYTES=10443
readonly EXPECTED_NUMSTAT_BYTES=5194
readonly EXPECTED_NAME_STATUS_BYTES=5011
readonly EXPECTED_CHANGED_PATHS=56
readonly EXPECTED_ADDED_PATHS=22
readonly EXPECTED_MODIFIED_PATHS=34

readonly EXPECTED_TI_JAVA_FILES=1704
readonly EXPECTED_TI_JAVA_MANIFEST_SHA256="f7ad843dcaec65afdb067df7e0849d0415a1d5f64fee13ba27088a8cc1ce632a"
readonly EXPECTED_TI_JAVA_ARCHIVE_BYTES=36945920
readonly EXPECTED_TI_JAVA_ARCHIVE_SHA256="80e9dea3e72d69b33dae8bf326cc3d7086a6bb093452ab25b1e9d88d84faf26a"
...
readonly EXPECTED_MINIPROGRAM_ARCHIVE_SHA256="6f65bdce1bf4fdb47edd1b417a65e4cf27b818ba792c896ea3ac89199d74bf92"
readonly EXPECTED_CONTRACT_BYTES=50467
readonly EXPECTED_CONTRACT_SHA256="2124d1b042f2df201ad3d8ca87fd19fa121b8d47cbaf51a60eb5271fe55b7fe8"
readonly EXPECTED_CONTRACT_PAYLOAD_SHA256="28f0fa1a5ec1c2e795c60d472b47d0ccb16d1b838a30dd0e7ac69fe738f53778"
readonly EXPECTED_WORM_BYTES=1442
readonly EXPECTED_WORM_SHA256="db1ffe2eaed03138fb75fd1007d032448960c502416ada92bec3d0846f4eaf0f"
readonly EXPECTED_BUILD_CONTEXT_SHA256="29372c7cb33edc16536d9fe10dacd1b7a5de669bcbcc8da21cc73496ce261ffc"
readonly EXPECTED_DOCKERFILE_SHA256="bb99afb7264a3a0d64b2e76d07a663bfe4a08cacca0387dff07635818a1ef499"
readonly EXPECTED_SUREFIRE_TESTS=860
readonly EXPECTED_FAILSAFE_TESTS=176
readonly EXPECTED_NODE_C_UNIT_TESTS=83
readonly EXPECTED_NODE_C_OPERATOR_ITS=3
readonly EXPECTED_NODE_C_RETRY_ITS=2
...
usage() {
    printf '%s\n' \
        ...
        "" \
        "Runs the fixed Node C implementation object in an isolated archive copy." \
...
verify_node_c_reports() {
    target="$1"
    started_epoch="$2"
    python3 - "$target" "$started_epoch" "$EXPECTED_NODE_C_UNIT_TESTS" \
        "$EXPECTED_NODE_C_OPERATOR_ITS" "$EXPECTED_NODE_C_RETRY_ITS" <<'PY'
import pathlib
import sys
import xml.etree.ElementTree as ET
...
expected_operator = int(sys.argv[4])
expected_retry = int(sys.argv[5])
unit_classes = {
    "io.saksk.ti.learning.infrastructure.migration.LegacyPersonalBankTagMigrationOperatorCoreStaticTest": 3,
    "io.saksk.ti.learning.infrastructure.migration.LegacyPersonalBankTagGlobalPreflightTest": 11,
    "io.saksk.ti.learning.infrastructure.migration.TagMigrationValueTypesTest": 45,
    "io.saksk.ti.learning.infrastructure.migration.BoundedSqlRetryTest": 24,
}
it_classes = {
    "io.saksk.ti.learning.infrastructure.migration.Phase4cLegacyPersonalBankTagOperatorCoreIT": expected_operator,
    "io.saksk.ti.learning.infrastructure.migration.Phase4cBoundedSqlRetryPostgresIT": expected_retry,
}
def verify(folder: str, classes: dict[str, int]) -> int:
    ...
    for class_name, expected in classes.items():
        ...
        if not path.is_file():
            raise SystemExit(f"missing Node C report: {path.name}")
        if path.stat().st_mtime < started_epoch - 1:
            raise SystemExit(f"stale Node C report: {path.name}")
        ...
        if actual != required:
            raise SystemExit(f"unexpected Node C report {path.name}: {actual}")
        total += actual["tests"]
    return total
if verify("surefire-reports", unit_classes) != expected_unit:
    raise SystemExit("Node C unit total mismatch")
if verify("failsafe-reports", it_classes) != expected_operator + expected_retry:
    raise SystemExit("Node C integration total mismatch")
PY
}
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/ti-phase4c-nodec-independent.XXXXXX")"
STAGE="$WORK/stage"
COPY="$STAGE/Ti-Java"
...
NODE_C_LOG="$WORK/maven-node-c.log"
...
PROJECT="ti-p4c-nodec-$LOWER_STAMP"
IMAGE="ti-java-phase4c-nodec:$LOWER_STAMP"
MAVEN_CACHE="ti-java-phase4c-nodec-m2-$LOWER_STAMP"
...
[ "$(byte_count "$COPY/docs/refactor/phase4c/personal-bank-tag-migration-operator-core-contract.json")" \
    -eq "$EXPECTED_CONTRACT_BYTES" ]
[ "$(hash_file "$COPY/docs/refactor/phase4c/personal-bank-tag-migration-operator-core-contract.json")" \
    = "$EXPECTED_CONTRACT_SHA256" ]
[ "$(byte_count "$COPY/docs/refactor/phase4c/personal-bank-tag-migration-operator-core-worm-evidence.json")" \
    -eq "$EXPECTED_WORM_BYTES" ]
[ "$(hash_file "$COPY/docs/refactor/phase4c/personal-bank-tag-migration-operator-core-worm-evidence.json")" \
    = "$EXPECTED_WORM_SHA256" ]
python3 - "$COPY/docs/refactor/phase4c/personal-bank-tag-migration-operator-core-contract.json" \
...
if hashlib.sha256(payload).hexdigest() != expected_payload:
    raise SystemExit("operator-core contract payload digest mismatch")
authority = document["source_authority"]
controls = set(authority["control_sources"])
fixed = set(authority["fixed_non_control_sources"])
if len(controls) != 7 or len(fixed) != 49 or controls & fixed:
    raise SystemExit("operator-core 7+49 source partition mismatch")
...
if controls | fixed != paths:
    raise SystemExit("operator-core 7+49 partition is not the exact C0 delta")
...
if any(route_state.get(key) != value for key, value in expected_route_state.items()):
    raise SystemExit("operator-core route state drifted")
PY
...
reports = {
    "surefire-reports": (
        "io.saksk.ti.learning.infrastructure.migration.LegacyPersonalBankTagMigrationOperatorCoreStaticTest",
        "io.saksk.ti.learning.infrastructure.migration.LegacyPersonalBankTagGlobalPreflightTest",
        "io.saksk.ti.learning.infrastructure.migration.TagMigrationValueTypesTest",
        "io.saksk.ti.learning.infrastructure.migration.BoundedSqlRetryTest",
    ),
    "failsafe-reports": (
        "io.saksk.ti.learning.infrastructure.migration.Phase4cLegacyPersonalBankTagOperatorCoreIT",
        "io.saksk.ti.learning.infrastructure.migration.Phase4cBoundedSqlRetryPostgresIT",
    ),
}
...
(
    ...
    TI_JAVA_MAVEN_CACHE_VOLUME="$MAVEN_CACHE" \
        ...
        -Dtest=LegacyPersonalBankTagMigrationOperatorCoreStaticTest,LegacyPersonalBankTagGlobalPreflightTest,TagMigrationValueTypesTest,BoundedSqlRetryTest \
        -Dit.test=Phase4cLegacyPersonalBankTagOperatorCoreIT,Phase4cBoundedSqlRetryPostgresIT \
        verify
) 2>&1 | tee "$NODE_C_LOG"
verify_node_c_reports "$COPY/server/target" "$FOCUSED_STARTED_EPOCH"
grep -Eq '^\[INFO\] Finished at: .*Z$' "$NODE_C_LOG"
...
verify_report_root
REPORT_TEMP="$(mktemp "$REPORT_ROOT/.phase4c-nodec-independent-report.XXXXXX")"
CAPTURED_AT="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
python3 - "$REPORT_TEMP" "$CAPTURED_AT" "$RUNNER_SHA256" "$RUNNER_BYTES" \
    "$REPORT_RELATIVE" "$PROJECT" "$IMAGE" "$STATIC_LOG" "$NODE_LOG" \
    "$DATA_PLANE_LOG" "$MAVEN_LOG" "$NODE_C_LOG" "$RUNTIME_SUMMARY" <<'PY'
import hashlib
import json
import pathlib
import sys
...
(output_raw, captured_at, runner_sha, runner_bytes, report_relative,
 ...
 node_c_log, runtime_summary) = sys.argv[1:]
...
document = {
    "schemaVersion": 1,
    "evidenceId": "ti.phase4c.tag-migration-operator-core-independent-acceptance.raw-v1",
    "status": "passed",
    "scope": "phase4c-learning-tag-migration-operator-core-fixed-c0-independent-copy",
    "capturedAt": captured_at,
    "authority": {
        "objectFormat": "sha1",
        "commitOid": "a70c365959e123950d30bff05adb4fabbb72d640",
        "parentOid": "bbeb08efcccb0b9974dfefa2044aab43e0675f6f",
        "rootTreeOid": "e23d0e8551dc8fd3338ba0682154eebe4a9e9f2f",
        "parentRootTreeOid": "2df48a21e622d0e5e3731fe2617ddaedbf466866",
        "tiJavaTreeOid": "38f6aafbeb7888a5539d6ab3b4b6b8d9ae522194",
        "parentTiJavaTreeOid": "ce2c2035763ac4512fa2bcaaa73cacb255212756",
        "serverTreeOid": "c4243039f367f7f917a3f9c85c190ebc5233a0fb",
        "parentServerTreeOid": "931e1268a43465023e23b31d903d5d7b3219981d",
        "serverSrcMainTreeOid": "bdd88effe149c61fada2300a4ec85bb2a3fdaf1c",
        "parentServerSrcMainTreeOid": "21fe4902d57a11998502e63041b5a56fb039a090",
        ...
        "changedPathCount": 56,
        "addedPathCount": 22,
        "modifiedPathCount": 34,
        "deletedPathCount": 0,
        "controlSourceCount": 7,
        "fixedNonControlSourceCount": 49,
        "standardRawSha256": "6892e663c1d1b5572c28c8aa489fc5721af4b4dbe7771edbbef12cf870bd5371",
        "standardNumstatSha256": "a42f9a7d5da81d41b08204bd26ffdbfc4da728f2541d417967c107d2f14dd87c",
        "standardNameStatusSha256": "2de8de0fd74bc734fe8cd4acb93dd50471b8179eb87e148c7f66e57188565d1d",
        "nulRawSha256": "ed145d808516678306ea1610fac1428244df90162371727ba9c1e4936381a196",
        "nulNumstatSha256": "f7b183cb1df7833418c014cd0a2d2a622dd0be43165e1dfa3dc6da5a7891d023",
        "nulNameStatusSha256": "22eb3e088ab24c2fcdb8defad142a01f6ba5a36bb81d9f0b5aec380a178c8725",
    },
    "runner": {
        "path": "tools/run_phase4c_tag_migration_operator_core_independent_acceptance.sh",
        "sha256": runner_sha,
    ...
    "fixedArchive": {
        ...
        "tiJava": {
            "fileCount": 1704,
            "archiveByteCount": 36945920,
            "archiveSha256": "80e9dea3e72d69b33dae8bf326cc3d7086a6bb093452ab25b1e9d88d84faf26a",
            "manifestSha256": "f7ad843dcaec65afdb067df7e0849d0415a1d5f64fee13ba27088a8cc1ce632a",
        },
        "miniprogram": {
            "fileCount": 630,
            "archiveByteCount": 10362880,
            "archiveSha256": "6f65bdce1bf4fdb47edd1b417a65e4cf27b818ba792c896ea3ac89199d74bf92",
        ...
        "contract": {
            "sha256": "2124d1b042f2df201ad3d8ca87fd19fa121b8d47cbaf51a60eb5271fe55b7fe8",
            "payloadSha256": "28f0fa1a5ec1c2e795c60d472b47d0ccb16d1b838a30dd0e7ac69fe738f53778",
            "byteCount": 50467,
        },
        "worm": {
            "sha256": "db1ffe2eaed03138fb75fd1007d032448960c502416ada92bec3d0846f4eaf0f",
            "byteCount": 1442,
        },
        "buildContextSha256": "29372c7cb33edc16536d9fe10dacd1b7a5de669bcbcc8da21cc73496ce261ffc",
    ...
    "verification": {
        ...
        "timezone": "UTC",
        "maven": {
            "surefire": {"tests": 860, "failures": 0, "errors": 0, "skipped": 0},
            "failsafe": {"tests": 176, "failures": 0, "errors": 0, "skipped": 0},
        },
        "focusedNodeC": {
            "unit": {"tests": 83, "failures": 0, "errors": 0, "skipped": 0},
            "operatorIntegration": {"tests": 3, "failures": 0, "errors": 0, "skipped": 0},
            "retryIntegration": {"tests": 2, "failures": 0, "errors": 0, "skipped": 0},
        },
        "logs": {
            ...
            "mavenFocusedNodeC": artifact(node_c_log),
        },
        "sourceDiscovery": {
    ...
    "closure": {
        "fixedC0IndependentCopyAcceptanceClosed": True,
        "provesOnlyCommit": "a70c365959e123950d30bff05adb4fabbb72d640",
        "provesC1EvidenceCommit": False,
        "provesC2AnchorCommit": False,
        "operatorCoreControlSourcesExternalGitAnchorComplete": False,
    },
}
...
if path.exists():
    shutil.rmtree(path)
PY
printf '%s\n' 'Phase 4C tag-migration operator-core independent acceptance passed'
