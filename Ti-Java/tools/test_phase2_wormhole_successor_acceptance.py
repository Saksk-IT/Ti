...
class Phase2WormholeSuccessorAcceptanceTest(unittest.TestCase):
    ...
    def test_fixed_ninth_node_appends_to_the_immutable_eighth_tip(self) -> None:
        ...
        self.assertEqual(
            PHASE4C_TAG_GLOBAL_PREFLIGHT_HARDENING_BUILD_CONTEXT_SHA256,
            seventh.build_context_sha256,
        )
        self.assertEqual(
            PHASE4C_TAG_GLOBAL_PREFLIGHT_HARDENING_DOCKERFILE_SHA256,
            seventh.dockerfile_sha256,
        )
        self.assertEqual(
            PHASE4C_TAG_GLOBAL_PREFLIGHT_REPORT_SHA256,
    ...
    def test_fixed_acceptance_requires_typed_normalization_anchor_successor(
        self,
    ) -> None:
        modules = {
            phase2_acceptance.READ_SUCCESSOR_MODULE: types.SimpleNamespace(
        ...
        with mock.patch.object(
            ...
            with self.assertRaisesRegex(
                EvidenceValidationError,
                "fixed HTTP typed-normalization anchor successor contract is required",
            ):
                phase2_acceptance.validate_fixed_acceptance(
                    self.root,
                    self.manifest_path,
    ...
    def test_fixed_acceptance_fails_when_typed_anchor_module_is_missing(
        self,
    ) -> None:
        modules = {
            phase2_acceptance.READ_SUCCESSOR_MODULE: types.SimpleNamespace(
        ...
        with mock.patch.object(
            ...
            with self.assertRaisesRegex(
                EvidenceValidationError,
                "fixed HTTP typed-normalization anchor successor acceptance module is required",
            ):
                phase2_acceptance.validate_fixed_acceptance(
                    self.root,
                    self.manifest_path,
