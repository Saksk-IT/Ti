...
schema dump、随机 Secret、容器、网络、卷和临时 Java 镜像会在退出时清理。仓库只保留不含 schema、DSN 或 Secret 的版本化报告；[`local-reference-verification.json`](local-reference-verification.json) 是不可变历史锚点，后续报告必须由 [`phase2_wormhole_successor_acceptance.py`](../../tools/phase2_wormhole_successor_acceptance.py) 以固定路径、报告 SHA、前驱 SHA、Dockerfile SHA 和 Java build-context SHA 显式加入后继链。当前固定链为 Phase 4B 锚点 → Phase 4C user-counts 入口 → Phase 4C HTTP-neutral read → Phase 4C HTTP-neutral read access → Phase 4C user-counts HTTP implementation。第五个节点 `personal-bank-user-counts-http-implementation-worm-evidence.json` 的报告 SHA-256 为 `7b863dd3b3bc94cbbfbd623d39495fed01c45dcb816598a759474d4372fbca39`，以前一不可变 tip `a393e79afb76c53a1aca8be1e4709506b58ad062e3c6536c26c12f10b29d1ec6` 为唯一前驱，并绑定 Java build-context `273227979fe0ef2efd1724e7f2e6b31b11ce19ebdcf0c262a1ff698dd8f158a3`。target-execution 与 typed-normalization 后继只新增测试、证据和门禁，生产 build-context 未变；固定链禁止重复 build-context，因此这些后继明确复用第五 tip，记录 `new_worm_report_created=false`，不伪造第六份报告。target-execution bootstrap、post-push、post-push external Git anchor、typed-normalization 与 typed-normalization external anchor 都是 WORM 链之外的固定验收节点；路由仍保持 `11 migrated / 600 pending / 0 cutover`，不得由这些证据自动晋级。静态门禁只接受固定五节点链的最后一个 tip，并同时要求固定 read successor、target-execution successor、post-push successor、post-push-anchor successor、`tools.phase4c_http_typed_normalization_successor_acceptance` 与 `tools.phase4c_http_typed_normalization_anchor_successor_acceptance` 的 `load(root)` 完整验收；六个模块均按代码固定名称惰性导入，不扫描目录，也不接受任意报告路径。`validate_fixed_chain` 则保持独立，供 terminal contract 构建时验证五节点物理证据而不反向导入任何 successor。所有报告仍仅代表获准本地开发参考实例，不确认生产版本，也不创建 Flyway baseline。
