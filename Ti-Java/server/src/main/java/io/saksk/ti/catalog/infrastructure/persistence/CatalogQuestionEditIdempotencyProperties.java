...
@ConfigurationProperties("ti.catalog.question-edit-idempotency")
public final class CatalogQuestionEditIdempotencyProperties {
    ...
    public CatalogQuestionEditIdempotencyProperties(String keySecret, Duration receiptTtl) {
        byte[] secretBytes = Objects.requireNonNull(keySecret, "keySecret")
                .getBytes(StandardCharsets.UTF_8);
        if (secretBytes.length < MINIMUM_SECRET_BYTES
                || secretBytes.length > MAXIMUM_SECRET_BYTES) {
            throw new IllegalArgumentException(
                    "Catalog question-edit idempotency key secret must contain between 32 and"
                            + " 1024 UTF-8 bytes");
        }
        ...
        if (receiptTtl.compareTo(MINIMUM_RECEIPT_TTL) < 0
            ...
            throw new IllegalArgumentException(
                    "Catalog question-edit receipt TTL must be between 5 minutes and 30 days");
        }
    ...
    public String toString() {
        return "CatalogQuestionEditIdempotencyProperties[keySecret=<redacted>, receiptTtl="
                + receiptTtl + "]";
    }
}
